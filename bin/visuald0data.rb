...
durchschnittverbrauch=read_durchschnittverbrauch(outputfolder_text+"/avaragevalues.txt",intervall)
...
Dir[logpath+"/"+logfiles].sort.each do |logfile|
  ...
  if durchschnittverbrauch.size <= verbrauch.size
    if durchschnittverbrauch.size < verbrauch.size
      printf("data size is with %d to big. using only %d values for avarage calculation.\n",verbrauch.size,durchschnittverbrauch.size)
    end
    durchschnittverbrauch.map!{|x| x*0.95}
    durchschnittverbrauch=durchschnittverbrauch.zip(verbrauch.map{|x| x*0.05}).map{ |x| x.reduce(:+) }
  else
    printf("Skipping avarage calculation while data size %d is not to small. it should be %d.\n",verbrauch.size,durchschnittverbrauch.size)
  end
  ...
  stats.close
  tmpfile.close
  avarage=3600*(allvalues.last-allvalues.first)/(intervall*allvalues.size)

  io=IO.popen("gnuplot", "w")
  io << initgnuplot
  io << sprintf("set output '%s'\n",graphfile)
  io << sprintf("set title 'Verbrauch am %04d-%02d-%02d (%.1fkWh)'\n",starttime.year,starttime.month,starttime.day,values[1]-startcounter)
  ...
  io << ","
  io << sprintf("     %f t 'Durchschnitt %.0fW'",avarage,avarage*1000)
  io << ","
...
tmpfile=File.new(outputfolder_text+"/avarageday.txt","w")
writeplotdata(tmpfile,durchschnittverbrauch,intervall_h)
tmpfile.close
avarage=durchschnittverbrauch.reduce(:+)/durchschnittverbrauch.size
io=IO.popen("gnuplot", "w")
io << initgnuplot
...
write_durchschnittverbrauch(outputfolder_text+"/avaragevalues.txt",durchschnittverbrauch)
